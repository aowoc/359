...
/* Clears the whole screen from top to bottom
 * Args:
 *  none
 * Return:
 *  none
...
ClearScreen:
	...
	mov		px, #0		// Set first block in row to be 0
	mov		py, #15		// Set last block in row to be 15
clearScreenLoop$:
	mov		r0, px		// Move px into r0 to be passed in (first block)
	mov		r1, py		// Move py into r1 to be passed in (last block) 
	ldr		r2,	=BG_COLOR 	// Set color for background
	bl		DrawCellRow	// Draw the row from first to last block
	
	add		px, #16		// Add by 16 to set get it into next row
	add		py,	#16		// Add by 16 to set get it into next row
	add		counter, #1	// Move x location to other side of sentance/word
	ldr		r0,	=16		// Load 16 into register so it could be compared
	cmp		counter, r0	// Compare the counter to 16
	blo		clearScreenLoop$	// Keep looping till all 16 rows are done
